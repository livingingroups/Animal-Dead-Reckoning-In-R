...
#m = Coefficient to multiply DBA by for speed estimate (e.g. The gradient (default = 1)). These can be changeable values (DBA~speed relationships according to movement modes) the length of other vector inputs (e.g., 'h') or a single value (one gradient used on all DBA values). 
...
#is retained and the process continues in an iterative style from this retained VP. If 'Dist_Head.corr' = TRUE, then the heading correction factor is also evaluated. But because its correction factor is of a different metric and scale, both the distance correction factor and heading correction factors
...
#User can opt for standard dead-reckoning (no VPC) by leaving 'VP.lat', 'VP.lon' and 'method' fields blank in the function input (default is NULL), 'VP.ME', 'bound', 'thresh.t', 'thresh.d', 'method', 'span', and 'Time_Dist_Corr.Fac' parameters within the function will also be ignored.
